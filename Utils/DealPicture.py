...
def rename_pic(read_path,filePath):
    # 用来判断程序消耗时间
    t1 = time.time()
                ...
                try:
                    ...
                    for index in range(len(casname_list)):
                        ...
                        if(new_name in saved_caslist):
                            ...
                            # 修改原来的名字为 100_10_1-1
                            cas_name=new_name+'_'+str(same_index)

                            new_file_name = filePath + '\\' + str(cas_name)+'.png'
                            shutil.copyfile('{old_file}'.format(old_file=src), '{new_file}'.format(new_file=new_file_name))
                            #记录已经存储过的cas号到saved_caslist
                            saved_caslist.append(new_name)
                        else:
